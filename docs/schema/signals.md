...
Every other background worker (vector indexing, search indexing, cache invalidation) has a native Rust implementation and can run either as its own process (`pylon worker start`) or in-process inside [`pylon-server`](../server.md). **Signals are the one exception** — a registered handler is a live Python object that only exists in a Python process, so `pylon-server` (pure Rust) never runs the dispatcher, under any flag combination.
