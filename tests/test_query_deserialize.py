...
def _named_tuple(name: str, position: int, members: list | None, type_name: str | None = None) -> dict:
    return {
        'kind': 'named_tuple',
        'name': name,
        'position': position,
...
class TestDecodeJsonTuple:
    ...
    def test_enum_member_hydrates_registered_enum(self):
        ...
        node = _named_tuple('', 0, [_member('status', kind='enum', enum_type='default::Status')])
