...
def _check_arguments(expected: set[str], kwargs: dict[str, Any]) -> None:
    ...
    Gel rejects a stray argument rather than ignoring it, and a silently
    dropped one hides what it usually is: a condition that was edited out, or
    a name that no longer matches. The wording is Gel's own, from
    `_make_missing_args_error_message` in its `object.pyx` codec, so a message
    carried over from an EdgeQL-era log or test still reads the same.
    """
    passed = set(kwargs)
    if not expected:
        if passed:
...
def _json_documents(rows: Any, compiled: CompiledQuery) -> list[str]:
    """Each row as the JSON document Gel would render for it, from the rows
