...
//! Live-Postgres tests for schema `Trigger`s — Pylon's analog of Gel's
//! `create trigger ... do (...)`. Scenarios adapted from Gel's own trigger
...
use common::*;
use pylon_core::diff::{diff_schema_steps, DbState};
...
async fn insert_writes_to_another_type() {
    // Gel: test_edgeql_triggers_insert_01
    ...
    let rows = rows_of(&pool, &schema, &format!("select {module}::Log {{ new_name }} filter .new_name = 'gadget'")).await;
    assert_eq!(rows.len(), 1, "trigger should have written exactly one Log row, got {rows:?}");
...
async fn delete_reads_old_row() {
    // Gel: test_edgeql_triggers_delete_01
    ...
    exec(&pool, &schema, &format!("insert {module}::Widget {{ name := 'doomed' }}")).await;
    exec(&pool, &schema, &format!("delete {module}::Widget filter .name = 'doomed'")).await;

    let rows = rows_of(&pool, &schema, &format!("select {module}::Log {{ old_name }} filter .old_name = 'doomed'")).await;
    assert_eq!(rows.len(), 1, "trigger should have captured the pre-delete name, got {rows:?}");
...
async fn update_reads_both_old_and_new() {
    // Gel: test_edgeql_triggers_update_01
    ...
    exec(&pool, &schema, &format!("insert {module}::Widget {{ name := 'before' }}")).await;
    exec(&pool, &schema, &format!("update {module}::Widget filter .name = 'before' set {{ name := 'after' }}")).await;

    let rows = rows_of(&pool, &schema, &format!("select {module}::Log {{ old_name, new_name }} filter .new_name = 'after'")).await;
    assert_eq!(rows.len(), 1, "trigger should have captured both old and new names, got {rows:?}");
...
async fn multiple_independent_triggers_all_fire() {
    // Gel: test_edgeql_triggers_mixed_01/02 — several triggers on one type,
    ...
    let mut widget = ty("Widget", &module, vec![id_prop(), text_prop("name")]);
    widget.triggers = vec![
        trigger(1, "After", &format!("insert {module}::Log {{ new_name := __new__.name }}")), // Insert-only
        trigger(6, "After", &format!("insert {module}::Log {{ old_name := __old__.name }}")), // Update|Delete
    ...
    exec(&pool, &schema, &format!("update {module}::Widget filter .name = 'w1' set {{ name := 'w2' }}")).await; // fires trigger 2 + 3
    exec(&pool, &schema, &format!("delete {module}::Widget filter .name = 'w2'")).await; // fires trigger 2 + 3

    let touched = rows_of(&pool, &schema, &format!("select {module}::Log {{ new_name }} filter .new_name = 'touched'")).await;
    assert_eq!(touched.len(), 3, "combined-event trigger should fire on every one of the 3 operations, got {touched:?}");

    let inserts = rows_of(&pool, &schema, &format!("select {module}::Log {{ new_name }} filter .new_name = 'w1'")).await;
    assert_eq!(inserts.len(), 1, "insert-only trigger should fire exactly once, got {inserts:?}");

    let old_names = rows_of(&pool, &schema, &format!("select {module}::Log {{ old_name }} filter .old_name = 'w1'")).await;
    assert_eq!(old_names.len(), 1, "update should have captured old_name='w1' once (from the update), got {old_names:?}");
    let old_names_w2 = rows_of(&pool, &schema, &format!("select {module}::Log {{ old_name }} filter .old_name = 'w2'")).await;
    assert_eq!(old_names_w2.len(), 1, "delete should have captured old_name='w2' once, got {old_names_w2:?}");
}

#[tokio::test]
#[ignore]
async fn trigger_updates_a_linked_row_of_another_type() {
    // Gel: test_edgeql_triggers_double_01 — not just an audit-log insert,
    ...
    let schema = SchemaDescriptor { types: vec![purchase, item], ..Default::default() };
    ...
    assert_eq!(field(&rows[0], 1), &CachedValue::I64(99), "trigger should have updated the linked Purchase's total");
}

#[tokio::test]
#[ignore]
async fn trigger_chaining_across_types() {
    // Gel: test_edgeql_triggers_chain_01 — an insert trigger on A inserts
    ...
    a.triggers = vec![trigger(1, "After", &format!("insert {module}::TypeB {{ tag := __new__.name }}"))];
    let b = ty("TypeB", &module, vec![id_prop(), text_prop("tag")]);
    let mut b = b;
    b.triggers = vec![trigger(1, "After", &format!("insert {module}::Log {{ new_name := __new__.tag }}"))];
    let schema = SchemaDescriptor { types: vec![a, b, log_type(&module)], ..Default::default() };

    let pool = test_pool().await;
    bootstrap(&pool).await;
    pool.batch_execute(&export_schema(&schema).unwrap()).await.unwrap();

    exec(&pool, &schema, &format!("insert {module}::TypeA {{ name := 'chain-test' }}")).await;

    let b_rows = rows_of(&pool, &schema, &format!("select {module}::TypeB {{ tag }} filter .tag = 'chain-test'")).await;
    assert_eq!(b_rows.len(), 1, "A's trigger should have inserted a TypeB row, got {b_rows:?}");

    let log_rows = rows_of(&pool, &schema, &format!("select {module}::Log {{ new_name }} filter .new_name = 'chain-test'")).await;
    assert_eq!(log_rows.len(), 1, "B's own trigger should have fired too (chained), got {log_rows:?}");
}
...
async fn before_trigger_does_not_block_the_operation_it_fires_on() {
    ...
    exec(&pool, &schema, &format!("insert {module}::Widget {{ name := 'before-delete' }}")).await;
    exec(&pool, &schema, &format!("delete {module}::Widget filter .name = 'before-delete'")).await;

    let remaining = rows_of(&pool, &schema, &format!("select {module}::Widget {{ name }} filter .name = 'before-delete'")).await;
    assert!(remaining.is_empty(), "the delete itself must still have gone through, got {remaining:?}");

    let log_rows = rows_of(&pool, &schema, &format!("select {module}::Log {{ old_name }} filter .old_name = 'before-delete'")).await;
    assert_eq!(log_rows.len(), 1, "Before trigger should still have fired and audited, got {log_rows:?}");
}
...
async fn migration_path_emits_the_same_working_trigger() {
    ...
    exec(&pool, &schema, &format!("insert {module}::Widget {{ name := 'via-diff' }}")).await;

    let rows = rows_of(&pool, &schema, &format!("select {module}::Log {{ new_name }} filter .new_name = 'via-diff'")).await;
    assert_eq!(rows.len(), 1, "trigger emitted via the diff path should work identically, got {rows:?}");
}
