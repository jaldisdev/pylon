...
//! Live-Postgres tests for property `Rewrite`s — Pylon's analog of Gel's
//! `create rewrite insert|update using (...)`. Scenarios adapted from Gel's
//! own `test_edgeql_rewrites.py` (noted per test), narrowed to Pylon's
...
async fn insert_rewrite_overrides_assigned_value() {
    // Gel: test_edgeql_rewrites_01 (insert half)
...
async fn update_rewrite_overrides_assigned_value() {
    // Gel: test_edgeql_rewrites_01 (update half)
...
async fn insert_rewrite_applies_to_defaulted_value() {
    // Gel: test_edgeql_rewrites_03 — interaction with a schema-level default:
...
async fn update_rewrite_references_sibling_property() {
    // Not a Gel-ported scenario — Pylon-specific: a rewrite expression that
...
async fn trigger_observes_rewritten_value_not_original() {
    ...
    // unlike a straight Gel port): an After-Insert trigger's __new__ must
