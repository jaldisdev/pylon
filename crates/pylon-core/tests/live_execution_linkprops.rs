...
//! multi-link (Pylon's analog of the upstream engine's `@prop` link properties) — writing
...
//! reading them back via `@prop` in a shape. Scenarios inspired by the upstream engine's own
//! the upstream linkprops suite, narrowed to what's directly portable — much
//! of that suite exercises upstream-only concepts (link unions, computed link
