...
use pylon_core::schema::{FunctionDescriptor, FunctionParamDescriptor, PropertyDescriptor, SchemaDescriptor, TypeDescriptor};
...
fn param(name: &str, pg_type: &str) -> FunctionParamDescriptor {
    FunctionParamDescriptor { name: name.into(), pg_type: pg_type.into() }
}
...
async fn object_set_returning_function_filters_correctly() {
    ...
    let sd = SchemaDescriptor { types: vec![person], functions: vec![adults], ..Default::default() };
    let pool = test_pool().await;
    bootstrap(&pool, &sd).await;
...
async fn function_composes_inside_a_larger_query() {
    ...
    let person = ty("Person", &module, vec![id_prop(), text_prop("name"), int_prop("age")]);
    let sd = SchemaDescriptor { types: vec![person], functions: vec![double], ..Default::default() };
    let pool = test_pool().await;
    bootstrap(&pool, &sd).await;

    exec(&pool, &sd, &format!("insert {module}::Person {{ name := 'Alice', age := 30 }}")).await;
    exec(&pool, &sd, &format!("insert {module}::Person {{ name := 'Bob', age := 10 }}")).await;

    // The function's result feeds directly into a filter on a different type.
    let rows = rows_of(&pool, &sd, &format!("select {module}::Person {{ name }} filter {module}::double(.age) > 40")).await;
    assert_eq!(rows.len(), 1, "only Alice (age 30 -> double 60) should match, got {rows:?}");
    assert_eq!(field(&rows[0], 1), &CachedValue::Str("Alice".to_string()));
}
...
async fn overload_resolution_by_argument_count() {
    ...
    let one_arg = FunctionDescriptor {
        name: "greet".into(), module: module.clone(),
        params: vec![param("name", "text")],
        return_pg_type: "text".into(), return_is_object: false, return_is_set: false, return_is_polymorphic: false,
        volatility: "immutable".into(), body: "'Hello, ' ++ name".into(),
    };
    let two_arg = FunctionDescriptor {
        name: "greet".into(), module: module.clone(),
        params: vec![param("greeting", "text"), param("name", "text")],
        return_pg_type: "text".into(), return_is_object: false, return_is_set: false, return_is_polymorphic: false,
        volatility: "immutable".into(), body: "greeting ++ ', ' ++ name".into(),
    };
    let sd = SchemaDescriptor { types: vec![], functions: vec![one_arg, two_arg], ..Default::default() };
    let pool = test_pool().await;
    bootstrap(&pool, &sd).await;
