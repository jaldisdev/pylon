...
use pylon_core::schema::{
    DeleteAction, DeleteSide, MultiLinkDescriptor, OnDeletePolicy, PropertyDescriptor, SchemaDescriptor, TypeDescriptor,
};
...
async fn single_link_default_restrict_blocks_delete_while_referenced() {
    ...
    assert!(
        result.is_err(),
        "deleting an Org still referenced by a Team (default Restrict) should fail"
    );
    ...
    assert_eq!(
        orgs.len(),
        1,
        "the Org must still exist after the restricted delete failed"
    );
}
...
async fn single_link_allow_sets_the_fk_null_on_target_delete() {
    ...
    exec(
        ...
        &schema,
        &format!(
            "insert {module}::TeamSetNull {{ name := 'Alpha', org := (select {module}::Org filter .name = 'Acme') }}"
        ),
    )
    .await;
    ...
    let teams = rows_of(
        &pool,
        &schema,
        &format!("select {module}::TeamSetNull {{ name, org: {{ name }} }} filter .name = 'Alpha'"),
    )
    .await;
    assert_eq!(
        teams.len(),
        1,
        "the Team itself must survive an Allow (SET NULL) target delete"
    );
...
async fn single_link_target_delete_source_cascades_to_the_team() {
    ...
    let teams = rows_of(
        &pool,
        &schema,
        &format!("select {module}::TeamCascadeOnTargetDelete filter .name = 'Alpha'"),
    )
    .await;
    assert_eq!(
        teams.len(),
        0,
        "DeleteSource (Target-side) must cascade-delete the Team when its Org is deleted"
    );
}
...
async fn single_link_source_delete_target_always_deletes_the_org() {
    ...
    exec(
        &pool,
        &schema,
        &format!("delete {module}::TeamDeleteTarget filter .name = 'Alpha'"),
    )
    .await;
    ...
    assert_eq!(
        orgs.len(),
        0,
        "DeleteTarget (Source-side) must delete the Org when the Team is deleted, regardless of other references"
    );
}
...
async fn single_link_source_delete_target_if_orphan_respects_other_references() {
    ...
    exec(
        &pool,
        &schema,
        &format!("insert {module}::Org {{ name := 'SoleOwned' }}"),
    )
    .await;
    for stmt in [
        format!(
            "insert {module}::TeamDeleteTargetIfOrphan {{ name := 'Alpha', org := (select {module}::Org filter .name = 'Shared') }}"
        ),
        format!(
            "insert {module}::TeamDeleteTargetIfOrphan {{ name := 'Beta', org := (select {module}::Org filter .name = 'Shared') }}"
        ),
        format!(
            "insert {module}::TeamDeleteTargetIfOrphan {{ name := 'Gamma', org := (select {module}::Org filter .name = 'SoleOwned') }}"
        ),
    ] {
    ...
    exec(
        &pool,
        &schema,
        &format!("delete {module}::TeamDeleteTargetIfOrphan filter .name = 'Alpha'"),
    )
    .await;
    ...
    assert_eq!(
        shared.len(),
        1,
        "Org still referenced by Beta must survive deleting Alpha"
    );
    ...
    exec(
        &pool,
        &schema,
        &format!("delete {module}::TeamDeleteTargetIfOrphan filter .name = 'Gamma'"),
    )
    .await;
    let sole_owned = rows_of(
        &pool,
        &schema,
        &format!("select {module}::Org filter .name = 'SoleOwned'"),
    )
    .await;
    assert_eq!(
        sole_owned.len(),
        0,
        "Org with no remaining references must be deleted when its sole-owning Team is deleted"
    );
}
...
async fn deleting_the_owner_always_cleans_up_its_own_junction_rows() {
    ...
    exec(
        &pool,
        &schema,
        &format!("delete {module}::ProductRestrictDefault filter .name = 'Widget'"),
    )
    .await;
    ...
    let products = rows_of(
        &pool,
        &schema,
        &format!("select {module}::ProductRestrictDefault filter .name = 'Widget'"),
    )
    .await;
...
async fn multilink_default_restrict_blocks_target_delete_while_referenced() {
    ...
    assert!(
        result.is_err(),
        "deleting a Tag still referenced by a Product (default Restrict) should fail"
    );
...
async fn multilink_allow_removes_the_junction_row_on_target_delete() {
    ...
    exec(
        ...
        &schema,
        &format!(
            "insert {module}::ProductAllow {{ name := 'Widget', tags := (select {module}::Tag filter .name = 'red') }}"
        ),
    )
    .await;
    ...
    let products = rows_of(
        &pool,
        &schema,
        &format!("select {module}::ProductAllow {{ name, tags: {{ name }} }} filter .name = 'Widget'"),
    )
    .await;
    assert_eq!(
        products.len(),
        1,
        "the Product itself must survive an Allow target delete"
    );
    ...
    assert_eq!(
        tags.len(),
        0,
        "the junction row must be gone, leaving an empty tags array"
    );
}
...
async fn multilink_target_delete_source_cascades_to_the_product() {
    ...
    let products = rows_of(
        &pool,
        &schema,
        &format!("select {module}::ProductCascadeOnTargetDelete filter .name = 'Widget'"),
    )
    .await;
    assert_eq!(
        products.len(),
        0,
        "DeleteSource (Target-side) must cascade-delete the Product when a referenced Tag is deleted"
    );
}
...
async fn multilink_source_delete_target_always_deletes_the_tag() {
    ...
    exec(
        &pool,
        &schema,
        &format!("delete {module}::ProductDeleteTarget filter .name = 'Widget'"),
    )
    .await;
    ...
    assert_eq!(
        tags.len(),
        0,
        "DeleteTarget (Source-side) must delete the Tag when the Product is deleted, regardless of other references"
    );
}
...
async fn multilink_source_delete_target_if_orphan_respects_other_references() {
    ...
    for stmt in [
        format!(
            "insert {module}::ProductDeleteTargetIfOrphan {{ name := 'Alpha', tags := (select {module}::Tag filter .name = 'shared') }}"
        ),
        format!(
            "insert {module}::ProductDeleteTargetIfOrphan {{ name := 'Beta', tags := (select {module}::Tag filter .name = 'shared') }}"
        ),
        format!(
            "insert {module}::ProductDeleteTargetIfOrphan {{ name := 'Gamma', tags := (select {module}::Tag filter .name = 'sole') }}"
        ),
    ] {
        exec(&pool, &schema, &stmt).await;
    }
    ...
    exec(
        &pool,
        &schema,
        &format!("delete {module}::ProductDeleteTargetIfOrphan filter .name = 'Alpha'"),
    )
    .await;
    ...
    assert_eq!(
        shared.len(),
        1,
        "Tag still referenced by Beta must survive deleting Alpha"
    );
    ...
    exec(
        &pool,
        &schema,
        &format!("delete {module}::ProductDeleteTargetIfOrphan filter .name = 'Gamma'"),
    )
    .await;
...
async fn migration_path_emits_working_deletion_policy_triggers() {
    ...
    assert!(
        ddl_ops
            .iter()
            .any(|op| op.contains("AFTER DELETE") || op.contains("BEFORE DELETE")),
    ...
    assert_eq!(
        orgs.len(),
        1,
        "Org still referenced by Beta must survive deleting Alpha (migration-built schema)"
    );
    ...
    assert_eq!(
        products.len(),
        0,
        "DeleteSource (Target-side) must cascade-delete the Product (migration-built schema)"
    );
}
