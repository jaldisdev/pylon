...
/// Gel's own `merge_iterator` puts between a chain of iterator CTEs.
...
mod tests {
    ...
    fn test_one_branch_of_an_object_if_else_is_empty() {
        // `{}` contributes no rows, so Gel's own rewrite (`SELECT A WHERE
    ...
    fn test_an_unreserved_keyword_names_a_free_shape_field() {
        // `last`, `first` and `order` are unreserved in EdgeQL, so a shape can
    ...
    fn test_a_backlink_tail_walks_past_the_first_step() {
        ...
        // Counted against the migrated data it agreed with Gel: 4 and 4.
    ...
    fn test_an_intersection_narrows_the_rest_of_a_multilink_walk() {
        ...
        // without a separate check. Agreed with Gel on real data (1 and 1).
