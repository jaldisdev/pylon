...
impl<'a> Compiler<'a> {
    ...
    /// compile-time tuple-index bounds check to match the upstream engine's own wording.
    ...
    fn compile_splat(
        ...
        if matches!(splat, ast::Splat::Deep) {
            for l in &td.links {
                ...
                // the upstream engine's `**` fetches every property (not further nested
    ...
    fn compile_expr_ctx(
        ...
        match expr {
            ...
            Expr::TypeCast(tc) => {
                ...
                // expression context, matching real PyQL semantics.
            ...
            Expr::TupleIndex { expr: inner, index } => {
                match inner.as_ref() {
                    ...
                    // (matches the upstream engine: `2 is not a member of tuple<std::int64, std::str>`).
                    _ => {
    ...
    fn resolve_name_ref(&self, name: &str, allow_fn_param: bool) -> Option<IrExpr> {
        ...
        // expose — matching the upstream engine (a free *object* needs an explicit shape to
...
/// (same type, or both-int, or both-float) doesn't cover — the upstream engine declares an
/// extensive set of these as real `CREATE INFIX OPERATOR` overloads (see
/// `the upstream datetime library`/`the upstream cal library`). Deliberately a separate,
