...
/// mirrors Gel's session config. Threaded from the client's `with_config()`
...
pub struct SessionConfig {
    ...
    /// to a primary-key ("id") property is a compile error — matches Gel's
...
/// `IrFreeSelect` type-level split: Gel/EdgeQL has no such distinction, and
...
pub enum IrExpr {
    ...
    /// the frontend needs to know which one it was to render Gel's
