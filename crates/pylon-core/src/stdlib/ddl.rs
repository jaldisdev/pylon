...
pub fn export_stdlib() -> String {
    ...
    out.push_str(concat!(
        ...
        "AS $$\n",
        "BEGIN\n",
        "    RAISE EXCEPTION '%', msg USING ERRCODE = 'invalid_parameter_value';\n",
        "END\n",
        "$$;\n\n",
        ...
        "AS $$\n",
        "DECLARE\n",
        "    parsed interval := val::interval;\n",
        "BEGIN\n",
        "    IF date_part('year', parsed) != 0 OR date_part('month', parsed) != 0\n",
        "        OR date_part('day', parsed) != 0 THEN\n",
        "        RAISE EXCEPTION 'invalid input syntax for type duration: %', quote_literal(val)\n",
        "            USING ERRCODE = 'invalid_datetime_format',\n",
        "                  HINT = 'Day, month and year units cannot be used for duration.';\n",
        "    END IF;\n",
        "    RETURN parsed;\n",
        "END\n",
        "$$;\n\n",
        "CREATE OR REPLACE FUNCTION _pylon.date_duration_in(val text)\n",
        "\tRETURNS interval\n",
        "\tLANGUAGE plpgsql IMMUTABLE PARALLEL SAFE STRICT\n",
        "AS $$\n",
        "DECLARE\n",
        "    parsed interval := val::interval;\n",
        "BEGIN\n",
        "    IF date_part('epoch', parsed - date_trunc('day', parsed)) != 0 THEN\n",
        "        RAISE EXCEPTION 'invalid input syntax for type cal::date_duration: %', quote_literal(val)\n",
        "            USING ERRCODE = 'invalid_datetime_format',\n",
        "                  HINT = 'Units smaller than days cannot be used for cal::date_duration.';\n",
        "    END IF;\n",
        "    RETURN parsed;\n",
        "END\n",
        "$$;\n\n",
